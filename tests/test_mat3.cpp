...
TEST_SUITE("mat3")
{
    ...
    TEST_CASE("Inversion")
    {
        ...
        auto B = A.invert();
        CHECK(approx_eq(B, Mat3<double>(0.101329955668144, 0.00633312222925902, -0.0348321722609246, 0.0139328689043699,
                                        0.125870804306523, -0.0672894236858771, -0.00823305889803673,
                                        -0.0630145661811273, 0.0965801139962001)));

        auto I = Mat3<double>::unit();
        CHECK(approx_eq((A.dot(B)), I));
        CHECK_THROWS_MESSAGE(Mat3<double>::zero().invert(), "this matrix is not invertible");
    }
